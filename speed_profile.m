...
for i = 1:length(max_front)
    if in_max == 0
        if max_front(i) == 1
            start = i;
            vel_profiles(sum(max_front(1:start)), 1:start) = integrate_backwards(path.s_m(1:start), path.k_1pm(1:start), a_max);
            in_max = 1;
        end
    else
        if max_rear(i) == 1
...
vel_profiles(end - 1, :) = const_accel(path.s_m, a_xmax);
...
for i = 1:(length(final_vel) - 1)
    final_ax(i) = (final_vel(i+1) - final_vel(i))/(path.s_m(i+1) - path.s_m(i)) * final_vel(i);
...
function Ux = integrate_backwards(s, K, a_max)
